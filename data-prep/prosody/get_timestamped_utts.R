...
gleason <- read_csv("data-prep/prosody/childes-audio-matching/gleason.csv") %>%
...
rollins <- read_csv("data-prep/prosody/childes-audio-matching/rollins.csv")
...
items <- read_csv("data-prep/item-info.csv") %>%
  pull(word)
...
for (i in items) {
  ...
  write_csv(timestamped, paste0("data-prep/prosody/timestamped/", i, ".csv"))
}