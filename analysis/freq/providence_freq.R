...
for (i in pairs) {
  ...
  plot <- providence_utterances %>%
    ...
    ggplot(aes(x=age, y=providence_freq, color=form, fill=form)) + 
...
ggsave("plots/freq/providence_raw_frequency.jpg", height = 15, width = 20, dpi = 300)
...
for (i in pairs) {
  ...
  plot <- providence_utterances %>%
    ...
    mutate(word = case_when(
      form == "ids" ~ paste(gsub("_.*", "", i)), 
      form == "ads" ~ paste(gsub(".*_", "", i)))) %>%
    ggplot(aes(x=age, y=providence_freq_relative, color=form, fill=form)) + 
    geom_vline(data = filter(aoa, word==paste(gsub("_.*", "", i))), mapping = aes(xintercept=aoa, color=form)) +
    geom_vline(data = filter(aoa, word==paste(gsub(".*_", "", i))), mapping = aes(xintercept=aoa, color=form)) +
    geom_point() +
    geom_smooth() +
...
ggsave("plots/freq/providence_per_item_frequency.jpg", height = 15, width = 12, dpi = 300)
...
ggsave("plots/freq/providence_props.jpg", height = 15, width = 12, dpi = 300)
...
ggsave("plots/freq/providence_odds.jpg", height = 15, width = 20, dpi = 300)
...
ggsave("plots/freq/providence_props_by_speaker.jpg", height = 15, width = 15, dpi = 300)
...
totaled_utterances <- merge(providence_utterances, month_totals, by="age")
...
ggsave("plots/freq/providence_per_age_frequency.jpg", height = 15, width = 20, dpi = 300)
