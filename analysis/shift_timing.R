...
early <- ggplot() + 
  geom_smooth((data=filter(model_data_long, pair %in% early_pairs)), mapping = aes(x=age, y=form_numeric, group=pair), 
              method="glm", method.args=list(family = "binomial"), 
...
late_model <- glmer(form_numeric ~ scale(age) + (1|pair) + (1|total_tokens),
                     family = "binomial",
                     control = glmerControl(optimizer="bobyqa"),
...
late <- ggplot() + 
  geom_smooth((data=filter(model_data_long, pair %in% late_pairs)), mapping = aes(x=age, y=form_numeric, group=pair), 
              method="glm", method.args=list(family = "binomial"), 
              color="#EDEDED", se=FALSE) +
...
never_model <- glmer(form_numeric ~ scale(age) + (1|pair) + (1|total_tokens),
                    family = "binomial",
                    control = glmerControl(optimizer="bobyqa"),
...
never <- ggplot() + 
  geom_smooth((data=filter(model_data_long, pair %in% never_pairs)), mapping = aes(x=age, y=form_numeric, group=pair), 
              method="glm", method.args=list(family = "binomial"), 
              color="#EDEDED", se=FALSE) +
