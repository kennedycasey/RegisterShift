...
for(i in items){
  ...
  get_ttr[[i]] <- utts_w_target
  
}

ttr <- do.call(rbind, get_ttr)
ttr$form <- factor(mlu$form, levels = c("ids", "ads"))

ttr %>%
  mutate(num_types = length(unique(strsplit(gloss, " ")))) %>%
  group_by(item, age) %>%
...
ggsave("plots/mlu/mlu_overall.jpg", height = 5, width = 4, dpi = 300)
