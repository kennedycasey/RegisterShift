...
local speech contexts surrounding CDL vs.~ADL variants in children's
...
\emph{stomach} to a child's vocabulary may mark their growing awareness
that word choice should be tailored to the current interactional context
(e.g., Clark, 1997, 2018). We do not, at present, know when children
begin to make shifts from CDL to ADL word use or precisely how such a
shift may be supported or initiated. Our investigation starts where this
transition is most easily observed, with CDL/ADL word pairs (e.g.,
\emph{doggy/dog}, \emph{night-night/goodnight}, \emph{tummy/stomach}),
as opposed to words that become less relevant with time (e.g.,
\emph{diaper} and \emph{peekaboo}).
...
Woodward, \& Kinzler, 2017; Soley \& Sebastian-Galles, 2020) suggest
...
from using CDL variants to using ADL variants? Our data suggest that the
average age of CDL-to-ADL switch occurs around 2.5 years.
...
own speech. Using the \emph{lme4} package in R (Bates, Mächler, Bolker,
...
vs.~ADL variants, with target child age (in months, scaled) as a single
...
captures, for each age, the probability of using ADL variants over CDL
variants.
...
items (Figure 2). In some cases, CDL variants were replaced by ADL
...
because ADL variants were already produced \textgreater50\% of the time
from the earliest ages sampled (e.g., \emph{duckie/duck} and
...
children's shifts away from CDL variants and toward ADL variants over
time as indirect evidence of their early formation of CDL and ADL as
distinct registers.

\hypertarget{study-2-what-linguistic-information-in-childrens-input-supports-their-shift-from-cdl-to-adl-variants}{%
\section{Study 2: What linguistic information in children's input
supports their shift from CDL to ADL
variants?}\label{study-2-what-linguistic-information-in-childrens-input-supports-their-shift-from-cdl-to-adl-variants}}
...
levels, including prosodic, lexical, and syntactic differentiation
...
We hypothesized that utterances with CDL variants, relative to
utterances with ADL variants, would be associated with (1) higher mean
pitch, (2) greater pitch variability, (3) slower speaking rates, and (4)
less lexical complexity. We also predicted that CDL utterances would
contain (5) fewer rare words, (6) fewer words overall, and (7) fewer
verb phrases. If these linguistic cues reliably differentiate CDL
vs.~ADL word usage contexts, then they could provide a viable source of
...
\caption[Coefficient estimates for linguistic predictors of variant in both others' and children's own utterances]{Coefficient estimates for linguistic predictors of variant in both others' and children's own utterances. Positive main effects indicate that utterances are more likely to contain ADL variants when they have higher values for that predictor (e.g., more verbs). Positive age interactions indicate an increasing effect of the predictor with age. Error bars depict standard errors of the coefficient estimates, and filled circles represent significant effects (\textit{p} $<$ 0.05).}\label{fig:ling-predictors-fig}
\end{figure*}
\end{CodeChunk}
...
We first analyzed 131,847 other-produced utterances (i.e., utterances
not produced by the target child) in the same CHILDES transcripts from
Study 1. The majority of utterances were produced by children's primary
caregivers (\emph{n} = 58,071, or 44.0\%). While, by and large, the
...
addressed to the target child. We followed up by examining children's
own productions from Study 1. However, Study 2 analyses exclude the LDP
corpus because it has not been comprehensively timestamped.
...
strength of association between a predictor and ADL variants. All main
effects of linguistic predictors and interactions with age are shown in
Figure 3. All models also revealed a positive main effect of target
child age (all \emph{p}s \textless{} 0.001), confirming that adults,
like children in Study 1, increase ADL variant production as their child
addressees get older.
...
complexity} (\(\beta\) = 0.27, \emph{SE} = 0.04, \emph{t} = 7.05,
\emph{p} \textless{} 0.001) and \textbf{lexical rarity} (\(\beta\) =
0.26, \emph{SE} = 0.01, \emph{t} = 22.54, \emph{p} \textless{} 0.001)
were more likely to contain ADL variants. Lexical complexity did not
interact with age (\(\beta\) = -0.01, \emph{SE} = 0.03, \emph{t} =
-0.22, \emph{p} = 0.827); whereas, lexical rarity negatively interacted
with age such that there was a decreasing effect of this predictor over
time (\(\beta\) = 0.03, \emph{SE} = 0.01, \emph{t} = 2.7, \emph{p} =
0.007).
...
were more likely to contain ADL variants. Moreover, both linguistic
predictors negatively interacted with age (Length: \(\beta\) = -0.16,
\emph{SE} = 0.01, \emph{t} = -14.61, \emph{p} \textless{} 0.001; Verbs:
\(\beta\) = -0.08, \emph{SE} = 0.01, \emph{t} = -7.51, \emph{p}
\textless{} 0.001), suggesting that the strength of these predictors
decreases across developmental time.

Across all levels of linguistic representation---prosodic, lexical, and
syntactic, children's productions largely mirrored others' (Figure 3).
That is, children's own utterances showed different patterns of
linguistic information for those containing CDL vs.~ADL variants, where
the latter were associated with wider pitch ranges (\(\beta\) = -0.04,
\emph{SE} = 0.02, \emph{t} = -2.28, \emph{p} = 0.023), faster speaking
rates (\(\beta\) = 0.13, \emph{SE} = 0.02, \emph{t} = 6.70, \emph{p}
\textless{} 0.001), greater lexical complexity (\(\beta\) = 0.16,
\emph{SE} = 0.05, \emph{t} = 3.07, \emph{p} = 0.002), more rare words
(\(\beta\) = 0.63, \emph{SE} = 0.01, \emph{t} = 51.63, \emph{p}
\textless{} 0.001), more words overall (\(\beta\) = 0.1, \emph{SE} =
0.01, \emph{t} = 7.75, \emph{p} \textless{} 0.001), and more verbs
(\(\beta\) = 0.05, \emph{SE} = 0.01, \emph{t} = 4.44, \emph{p}
\textless{} 0.001), but not significantly with lower mean pitch
(\(\beta\) = -0.02, \emph{SE} = 0.02, \emph{t} = -1.22, \emph{p} =
0.221).
...
the differences in the contexts of CDL vs.~ADL variant use and thereby
supporting their gradual transition away from production of more
...
glossing over this lexical variation may present a missed opportunity to
investigate more nuanced aspects of vocabulary development. The present
findings on the transition between CDL and ADL variants help demonstrate
...
under which new words are introduced (e.g., Weatherhead, Kandhadai,
Hall, \& Werker, 2021), and here, too, children's shifts from CDL to ADL
vocabulary are likely supported by their emerging knowledge about the
contexts in which different registers are used.

We focus in the present study on the issue of encountering multiple
labels for the same referent in early word learning, but children also
face the inverse problem---one label for many different referents (e.g.,
...
as interrelated---and given children's early success in contending with
both sources of variability, as evidence that learning is happening at
multiple levels.
...
infants' first words include uh-oh and hi. \emph{PsyArXiv}.
